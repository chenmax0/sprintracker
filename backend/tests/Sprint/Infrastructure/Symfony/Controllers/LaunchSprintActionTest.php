...
class LaunchSprintActionTest extends WebTestCase
{
    use ResetsDatabase;
    use AppTestHelpers;
    ...
    public function testAnyTeamMemberCanLaunchSprint(): void
    {
    ...
    public function testCannotLaunchASecondSprintWhileOneIsActive(): void
    {
        $ownerToken = $this->registerAndLogin('jane@example.com');
    ...
    public function testSprintNumberIncrementsAfterCompletingThePrevious(): void
    {
        $ownerToken = $this->registerAndLogin('jane@example.com');
        $projectId = $this->createTeamAndProject($ownerToken);
        $firstSprintId = $this->createSprint($ownerToken, $projectId);

        $this->client->request('POST', "/api/sprints/$firstSprintId/complete", server: [
            'CONTENT_TYPE' => 'application/json',
            'HTTP_AUTHORIZATION' => 'Bearer '.$ownerToken,
    ...
    public function testNonMemberCannotLaunchSprint(): void
    {
        $ownerToken = $this->registerAndLogin('jane@example.com');
